...
export function CreateProductForm({ categories }: CreateProductFormProps) {
  ...
  const createProductMutation = useMutation({
    ...
    onSuccess: (createdProduct) => {
      toast.success(t('productsCreateSuccess', { title: createdProduct.title }))
      router.push('/admin/products')
    },
    onError: (error) => {
      const message = error instanceof ApiError ? error.message : t('productsCreateError')
      toast.error(message)
    },
  })
  ...
  const isFormDisabled = isSubmitting || createProductMutation.isPending
      ...
      <div className="space-y-8">
        ...
        <div className="flex items-center gap-3 border-t border-border pt-6">
          <Button type="submit" disabled={isFormDisabled}>
            {isFormDisabled ? t('productsFormSubmitting') : t('productsFormSubmit')}
          </Button>
          <Button
            type="button"
            variant="outline"
