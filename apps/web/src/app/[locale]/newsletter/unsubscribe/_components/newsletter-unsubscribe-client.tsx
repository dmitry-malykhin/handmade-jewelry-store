...
type UnsubscribeState = 'pending' | 'success' | 'already' | 'error'
...
export function NewsletterUnsubscribeClient() {
  ...
  const [state, setState] = useState<UnsubscribeState>('pending')
  ...
  useEffect(() => {
    ...
    unsubscribeFromNewsletter(email, token)
      .then((result) => setState(result.status === 'already-unsubscribed' ? 'already' : 'success'))
